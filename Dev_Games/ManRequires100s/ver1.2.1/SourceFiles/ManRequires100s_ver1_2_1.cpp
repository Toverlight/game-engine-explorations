...
#define ENEMY_NUM (6)//敌人基本数量，随难度调整
...
//初始化游戏
void startup(Info* info,Player* player,Enemy enemies[],Obstacle obstacles[])
{
...
//显示
void show(Info* info,Player* player,Enemy enemies[],Obstacle obstacles[])
{
	gotoxy(0, 0);
	//显示边界、玩家和敌群
	static int j, i;
	...
	for (u = 0; u < obstacleNum; u++)
	{
		...
		if (obstacles[u].ch == 100)
		{
			printf("\u25A1");
		}
		else if (obstacles[u].ch == 101)
		{
			printf("\u25A0");
		}
		else
		{
			printf("%c", obstacles[u].ch);
		}
	}
	//显示坚持的秒数
	gotoxy(0, HEIGHT);
...
//与用户输入无关的更新
void updateWithoutInput(Info* info,Player* player,Enemy enemies[],Obstacle obstacles[])
{
	static int temp_x, temp_y, reFlag = 1,randFlag = 0,initRandFlagRate = 0,account = 0,speedup = 0;
	static int k,pre_x,pre_y;
	//每个敌人的位置更新
	for (k = 0; k < ENEMY_NUM * info->difficulty; k++)
	{
...
//与用户输入有关的更新
void updateWithInput(Player* player,Obstacle obstacles[])
{
...
//游戏结束的处理（赢或输）
int Process(Info* info,Enemy enemies[],Obstacle obstacles[])
{
	//敌群和障碍物动态数组内存的回收
	free(enemies);
	free(obstacles);
...
Restart:
	...
	startup(&info,&player,enemies,obstacles);
	...
	while ((info.failFlag == 0) && (info.ms < 100 * 1000))
	{
		...
		show(&info,&player,enemies,obstacles);
		updateWithoutInput(&info,&player,enemies,obstacles);
		updateWithInput(&player,obstacles);
		Delay();
	}
	int restart = Process(&info,enemies,obstacles);
	if (restart)
	{
		system("cls");
		goto Restart;
