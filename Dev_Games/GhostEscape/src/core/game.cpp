...
void Game::run()
{
    while (isRunning_) {
        ...
        if (elapsed < frameDelay_) {
            ...
            dt_ = static_cast<float>(frameDelay_ / 1.0e9);
        } else {
...
{
    SDL_FRect src_rect = { 
        texture.src_rect.x, texture.src_rect.y + texture.src_rect.h * (1 - mask.y), 
        texture.src_rect.w * mask.x, texture.src_rect.h * mask.y
    };
    SDL_FRect dst_rect = { 
        position.x, position.y + size.y * (1 - mask.y), 
        size.x * mask.x, size.y * mask.y
    };
...
void Game::renderHBar(const glm::vec2 &position, const glm::vec2 &size, float percent, SDL_FColor color)
{
    ...
    SDL_FRect fill_rect = { position.x, position.y, size.x * percent, size.y };
...
{
    return (mouse_position_.x >= top_left.x && mouse_position_.x <= bottom_right.x &&
            mouse_position_.y >= top_left.y && mouse_position_.y <= bottom_right.y);
}
...
{
    mouse_button_state_ = SDL_GetMouseState(&mouse_position_.x, &mouse_position_.y);
    // 限制比例，不出现黑边的做法
    int w, h;
