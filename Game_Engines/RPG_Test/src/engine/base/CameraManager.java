...
public class CameraManager {
    ...
    public static void addCamera(Camera camera) {
        cameras.add(camera);
        if (LogUtils.isLoggable(Level.INFO)) {
            LogUtils.info("Camera(id %s) named '%s' was added.", camera.getId(), camera.getName());
        }
    }
    ...
    public static void removeCamera(Camera camera) {
        if (cameras.remove(camera)) {
            LogUtils.info("Camera(id %s) named '%s' was removed.", camera.getId(), camera.getName());
        } else {
            LogUtils.warning("Camera(id %s) named '%s' wasn't contained by [list]!", camera.getId(),
                    camera.getName());
        }
    }
    ...
    public static void setMainCamera(String nameOfMainCamera) {
        for (Camera camera : cameras) {
            if (camera.getName().equals(nameOfMainCamera)) {
                ...
                if (LogUtils.isLoggable(Level.INFO)) {
                    LogUtils.info("MainCamera changed, (id %s) named '%s'.", mainCamera.getId(),
                            nameOfMainCamera);
                }
                return;
            }
        ...
        if (LogUtils.isLoggable(Level.WARNING)) {
            LogUtils.warning("No camera found in [list] and mainCamera ref failed to change, " +
