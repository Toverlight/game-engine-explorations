    ...
    public void start() {
        if (gameObject != null) {
            ...
            if (LogUtils.isLoggable(Level.FINER)) {
                LogUtils.finer("PositionOrbiting(id %s) managed to be added to gameObject(id %s) named '%s'.",
                        getId(), gameObject.getId(), gameObject.getName());
            }
        } else {
            LogUtils.severe("PositionOrbiting(id %s) failed to find its belonging gameObject and " +
